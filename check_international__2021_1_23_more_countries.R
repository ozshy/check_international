...
dim(frac_vol2.df)# num of countries
...
### 2015-2019 Volume graph (% time series, ts_high, of high 6 countries)
str(frac_vol2.df)
frac_vol2.df
frac_vol3.df = frac_vol2.df
rownames(frac_vol3.df)
(rownames(frac_vol3.df) = frac_vol3.df$Country)
colnames(frac_vol3.df)
(frac_vol4.df = subset(frac_vol3.df, select = -Country))


# transpose to have Countries as columns
(frac_vol5.df = as.data.frame(t(frac_vol4.df)))
colnames(frac_vol5.df)
row.names(frac_vol5.df)
dim(frac_vol5.df)
str(frac_vol5.df)
#
# selecting 6 countries with high use in 2019
(frac_vol_high1.df = frac_vol5.df[, c("United States", "France", "Mexico", "India", "Canada", "Argentina")])
...
(frac_vol_low1.df = frac_vol5.df[, c("Italy", "Brazil", "United Kingdom", "Spain", "Austria", "Korea")])
...
### computing growth rates volume and value for 12 contries displayed in Figures 3 and 4 (time series vol and val)
...
check_growth3.df
(check_growth4.df = check_growth3.df[-c(6, 14, 15, 19), ])
dim(check_growth4.df)
rownames(check_growth4.df)=1:18
str(check_growth4.df)
(check_growth5.df = droplevels(check_growth4.df))
# add Japan (removed earlier because data lack total number of cashless papers). Recall:
check_vol2.df
(japan_vol.vec = check_vol2.df[check_vol2.df$Country=="Japan",])
(check_growth6.df = rbind(check_growth5.df, japan_vol.vec))
str(check_growth6.df)
(check_growth7.df = check_growth6.df[order(as.character(check_growth6.df$Country)), ])
str(check_growth7.df)
#
# constructing vector with 2019 population size (millions)
pop.vec = c(44.9, 8.8, 11.5, 211.0, 37.6, 67.1, 83.0, 1366.0, 270.6, 60.4, 126.3, 51.7, 127.6, 34.3, 58.6, 46.9, 82.0, 66.6, 328.2)
length(pop.vec)
